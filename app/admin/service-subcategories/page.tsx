...
import { getServiceSubcategories, addServiceSubcategory, updateServiceSubcategory, deleteServiceSubcategory, getServiceCategories, type ServiceSubcategory, type ServiceCategory } from '@/lib/mockDatabase';
import { toast } from 'sonner';

export default function ServiceSubcategoriesPage() {
  const [subcategories, setSubcategories] = useState<ServiceSubcategory[]>(getServiceSubcategories());
  const [categories] = useState<ServiceCategory[]>(getServiceCategories());
  ...
  const handleAddSubcategory = () => {
    ...
    try {
      const newSubcategory = addServiceSubcategory(formData);
      setSubcategories(getServiceSubcategories());
      setIsAddDialogOpen(false);
      resetForm();
      toast.success('Service subcategory added successfully');
    } catch (error) {
      toast.error('Failed to add service subcategory');
    }
  };
  ...
  const handleUpdateSubcategory = () => {
    ...
    try {
      updateServiceSubcategory(editingSubcategory.id, formData);
      setSubcategories(getServiceSubcategories());
      setEditingSubcategory(null);
      resetForm();
      toast.success('Service subcategory updated successfully');
    } catch (error) {
      toast.error('Failed to update service subcategory');
    }
  };
  ...
  const handleDeleteSubcategory = (subcategoryId: string) => {
    if (confirm('Are you sure you want to delete this service subcategory?')) {
      try {
        deleteServiceSubcategory(subcategoryId);
        setSubcategories(getServiceSubcategories());
  ...
  const handleToggleStatus = (subcategoryId: string, is_active: boolean) => {
    try {
      updateServiceSubcategory(subcategoryId, { is_active });
      setSubcategories(getServiceSubcategories());
      toast.success(`Service subcategory ${is_active ? 'activated' : 'deactivated'} successfully`);
    } catch (error) {
      toast.error('Failed to update service subcategory status');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Service Subcategories ({filteredSubcategories.length})</CardTitle>
