...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/products/slug/${slug}`, { headers: { 'Accept': 'application/json' } })
      .then((res) => {
    ...
    return (
      <div className="bg-background min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/products" className="inline-flex items-center px-5 py-2.5 bg-accent text-accent-foreground rounded-xl font-medium hover:bg-accent/90 transition-colors">
            ← Back to Products
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              ...
              {product.model && <p className="text-muted-foreground mt-1 text-sm">Model: {product.model}</p>}
            </div>

            {/* Price */}
            <div>
              <div className="flex items-baseline gap-3">
                <span className="text-4xl font-bold text-foreground">${effectivePrice.toLocaleString()}</span>
                {isOnSale && (
                  <span className="text-xl text-muted-foreground line-through">${basePrice.toLocaleString()}</span>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm border border-border rounded-xl p-4 bg-muted/30">
              {product.code && (
                <><span className="text-muted-foreground">Product Code</span><span className="font-medium">{product.code}</span></>
              )}
              {product.sku && (
                <><span className="text-muted-foreground">SKU</span><span className="font-medium">{product.sku}</span></>
              )}
              {product.subcategories?.length ? (
                <><span className="text-muted-foreground">Subcategory</span><span className="font-medium">{product.subcategories.map((s) => s.name).join(', ')}</span></>
              ) : null}
              {availableStock !== null && (
                <>
                  <span className="text-muted-foreground">Availability</span>
                  <span className={`font-medium ${availableStock > 0 ? isLowStock ? 'text-yellow-600' : 'text-green-600' : 'text-red-500'}`}>
                    {availableStock > 0 ? isLowStock ? `Low Stock (${availableStock} left)` : `In Stock (${availableStock})` : 'Out of Stock'}
                  </span>
                </>
              )}
            </div>
        ...
        {product.description && (
          <div className="mt-16 border-t border-border pt-10">
            ...
            <p className="text-muted-foreground leading-relaxed whitespace-pre-line max-w-4xl">{product.description}</p>
          </div>
        )}
